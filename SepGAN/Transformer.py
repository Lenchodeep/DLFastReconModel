"""
    ...
    source code: https://github.com/chunmeifeng/MTrans 
"""
...
class Attention_Encoder(nn.Module):
    def __init__(self, dim, kv_reduced_dim=None, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.,
                 proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads                    ##NOTE  multihead attention里面的header的个数
        head_dim = dim // num_heads                   ##NOTE 每个head 有多少channel的特征图
        # NOTE scale factor was wrong in my original version, can set manually to be compat with prev weights
        self.scale = qk_scale or head_dim ** -0.5     ##NOTE  求取attention的缩放因子d

        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)  ##NOTE 用一个全连接层来代替对于输入x进行三次全连接获取q，k，v的过程
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)                  ##NOTE 输出映射的全连接层
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, input):
        ...
        h = H//self.patchSize
        w = W//self.patchSize
        input = torch.reshape(input, (B,(h*w), (C*self.patchSize*self.patchSize)))
        return input
...
class Block_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm2(x)
        x = self.mlp(x)
        x_residual = self.drop_path(x)

        out  = input + x_residual
        return out


class VisionTransformer(nn.Module):
    def __init__(self, embed_dim=768, depth=4,                             ##NOTE depth：vit中encoder的个数
                  num_heads=8, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm):
        super().__init__()

        self.pos_drop = nn.Dropout(p=drop_rate)   ##NOTE attention之前的drop 

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # dropout的ratio是递增的
