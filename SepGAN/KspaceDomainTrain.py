...
def train(args):
    ...
    D_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(G_optimizer, 'min', patience = 5, factor=0.1) ## min 表示当监控的量不在下降的时候，降低学习率， patience表示当性能在5个epoch后仍然不下降，降低学习率
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            with tqdm(desc=f'Epoch {epoch + 1}/{args.epochs}', unit=' imgs') as pbar:
                ...
                for step,batch in enumerate(train_loader):
                    ...
                    #calculate G loss
                    FullLoss, ImL2, ImL1, kL2, advLoss , ssimLoss, midssimLoss, gradientLoss = \
                                        loss.calc_gen_loss(rec_img, rec_mid_img,rec_k, target_img, target_k, D_fake_pred, True)
                    ...
                    epoch_mid_SSIM += midssimLoss.item()
                    ...
                    if args.GAN_training:
                        pbar.set_postfix(**{'FullLoss': FullLoss.item(),'ImL2': ImL2.item(), 'ImL1': ImL1.item(), 'SSIM': ssimLoss.item(), 'mid_SSIM': midssimLoss.item(),"GradientLoss":gradientLoss.item(),
                                            'KspaceL2': kL2.item(),'Adv G': advLoss.item(),'Adv D - Real' : D_real_loss.item(),'PSNR: ' : psnr,
...
###
            # On epoch end
            # Validation
            val_rec_img, val_full_img, val_rec_mid_image, val_FullLoss, val_ImL2, val_ImL1, val_KspaceL2, val_PSNR, val_ssim, valmidssim, val_gradient =\
                psycho_eval_net1(G_model, val_loader, loss, args.device)
            logging.info('Validation full score: {}, ImL2: {}. ImL1: {}, KspaceL2: {}, PSNR: {}, SSIM: {}, MID_SSIM: {}, "GradientLoss: " {}'
                         .format(val_FullLoss, val_ImL2, val_ImL1, val_KspaceL2, val_PSNR, val_ssim, valmidssim, val_gradient))
            # Schedular update
            ...
            if args.tb_write_loss:
                ...
                writer.add_scalar('train/midSSIM', epoch_mid_SSIM / len(train_loader), epoch)
                ...
                writer.add_scalar('validation/midSsimLoss', val_ssim, epoch)
