...
class IXIDatasetRefinrGan(Dataset):
    '''
    ...
    def __getitem__(self, i):
        ...
        if self.is_train:
            ...
            full_file_path_B = path.join(self.data_dir,file_name_B + '.hdf5')

            with h5py.File(full_file_path_B, 'r') as f:
                imageB = f['data'][:, :, slice_num_B]
            ...
            full_file_path = path.join(self.data_dir,val_file_name + '.hdf5')

            with h5py.File(full_file_path, 'r') as f:
                val_imageA = f['data'][:, :, val_slice_num]
...
def main():
    ...
    dataset = IXIDatasetRefinrGan('D:/datas/IXI-T1-save/', args=args,isTrainData=True ,transform=None)    
