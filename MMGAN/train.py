...
def train(args):
    ...
    train_dataset = FastMRIDataset(args, True, transform)
    val_dataset = FastMRIDataset(args, False, transform)
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            with tqdm(desc=f'Epoch {epoch + 1}/{args.epochs}', unit=' imgs') as pbar:
                ...
                for step,batch in enumerate(train_loader):
                    ...
                    #calculate G loss
                    FullLoss, IMloss_T1, IMloss_T2, KLoss_T1, KLoss_T2, SSIMLoss_T1, SSIMLoss_T2, advLoss\
                     = loss.calc_gen_loss(T1_target_img, T2_target_img, rec_img_T1, rec_img_T2,\
                                        T1_target_k, T2_target_k, rec_k_T1,rec_k_T2, D_fake_pred)
                    ...
                    pbar.set_postfix(**{'FullLoss': FullLoss.item(),'ImT1': IMloss_T1.item(), 'ImT2': IMloss_T2.item(),
                                    "KLoss_T1 ":KLoss_T1.item(), 'KLoss_T2': KLoss_T2.item(),'Adv G': advLoss.item(), 'Adv D': DLoss.item(),
